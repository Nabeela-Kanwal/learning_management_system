                    ...
                    <div class="row">
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <select id="{{ $field }}" name="{{ $field }}" class="form-select" required>
                                    ...
                                    @foreach ($options as $option)
                                        <option value="{{ $option->id }}" @if ($field === 'subcategory_id') data-category="{{ $option->category_id }}" @endif @selected(old($field, $course->$field ?? '') == $option->id)>{{ $option->name }}{{ $field === 'instructor_id' ? ' ('.$option->email.')' : '' }}</option>
                                    @endforeach
                                </select>
                            </div>
                        @endforeach
                        @foreach (['course_title' => 'Course Title', 'course_slug' => 'Course Slug', 'course_name' => 'Course Name', 'video_url' => 'Video URL', 'label' => 'Level', 'resources' => 'Resources', 'certificate' => 'Certificate', 'selling_price' => 'Selling Price', 'discount_price' => 'Discount Price'] as $field => $label)
                            <div class="col-md-6 mb-3">
                                <label for="{{ $field }}" class="form-label">{{ $label }}</label>
                                <input id="{{ $field }}" name="{{ $field }}" class="form-control" value="{{ old($field, $course->$field ?? '') }}"
                        ...
                        <div class="col-12 mb-3">
                            ...
                            @if (!empty($course->course_image))
                                <div><img src="{{ asset($course->course_image) }}" alt="Current course image" class="rounded mb-2" style="max-width: 180px;"></div>
                            @endif
                            <input type="file" id="course_image" name="course_image" class="form-control" accept="image/jpeg,image/png,image/webp">
                            <div class="form-text">JPG, PNG or WebP, up to 2 MB. Leave empty to keep the current image.</div>
                        </div>
                        @foreach (['course_description' => 'Description', 'prerequisites' => 'Prerequisites'] as $field => $label)
                            <div class="col-12 mb-3">
                                <label for="{{ $field }}" class="form-label">{{ $label }}</label>
                                <textarea id="{{ $field }}" name="{{ $field }}" class="form-control" rows="4" @if ($field === 'prerequisites') maxlength="500" @endif>{{ old($field, $course->$field ?? '') }}</textarea>
                            </div>
                        @endforeach
                        <div class="col-12 mb-3">
                            @foreach (['best_seller' => 'Best Seller', 'featured' => 'Featured', 'hightest_rated' => 'Highest Rated'] as $field => $label)
                                ...
                                <div class="form-check form-check-inline">
                                    <input type="checkbox" id="{{ $field }}" name="{{ $field }}" value="1" class="form-check-input" @checked(old($field, $course->$field ?? 0) == 1)>
