...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y course-detail">
        ...
        <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
            <div>
                <a href="{{ route('admin.courses.index') }}" class="text-muted small"><i class="bx bx-left-arrow-alt"
                        aria-hidden="true"></i> Back to Courses</a>
                <h4 class="fw-bold mb-0 mt-2">Course Details</h4>
            </div>
            <a href="{{ route('admin.courses.edit', $course->id) }}" class="btn btn-primary"><i class="bx bx-edit-alt me-2"
                    aria-hidden="true"></i> Edit Course</a>
        </div>
            ...
            <div class="card-body d-flex align-items-center gap-4 course-hero-content">
                ...
                <div class="min-width-zero">
                    <div class="d-flex align-items-center flex-wrap gap-2 mb-2">
                        ...
                        <span
                            class="badge {{ $course->status == 1 ? 'bg-label-success' : 'bg-label-danger' }}">{{ $course->status == 1 ? 'Active' : 'Inactive' }}</span>
                    ...
                    <p class="mb-2 text-break">{{ $course->category?->name ?? 'Category unavailable' }} <span
                            class="text-muted mx-1">/</span> {{ $course->subcategory?->name ?? 'Subcategory unavailable' }}
                    </p>
                    <div class="text-muted small"><i class="bx bx-calendar me-1" aria-hidden="true"></i> Created
                        {{ $course->created_at?->format('M d, Y') ?? 'date unavailable' }}</div>
                </div>
            </div>
        </div>
                    ...
                    <div class="card-body">
                        ...
                        <dl class="row g-3 course-meta mb-4">
                            @foreach (['course_name' => 'Course Name', 'course_slug' => 'Slug', 'label' => 'Level'] as $field => $label)
                                <div class="col-sm-6">
                                    <dt>{{ $label }}</dt>
                                    <dd>{{ $course->$field ?: 'Not provided' }}</dd>
                                </div>
                            @endforeach
                        </dl>
                        <hr class="my-4">
                        ...
                        <div class="course-prose">
                            {{ strip_tags($course->course_description ?: 'No description provided.') }}</div>
                        <hr class="my-4">
                        <h6>Prerequisites</h6>
                        <div class="course-prose">{{ strip_tags($course->prerequisites ?: 'No prerequisites provided.') }}
                        </div>
                    </div>
                </section>
                <section class="card mb-4">
                                ...
                                <dd>
                                    @if ($course->video_url && in_array(strtolower(parse_url($course->video_url, PHP_URL_SCHEME) ?? ''), ['http', 'https']))
                                        <a href="{{ $course->video_url }}" target="_blank"
                                            rel="noopener noreferrer">{{ $course->video_url }} <i
                                                class="bx bx-link-external" aria-hidden="true"></i><span
                            ...
                            @foreach (['selling_price' => 'Selling Price', 'discount_price' => 'Discount Price'] as $field => $label)
                                <div class="col-6">
                                    <div class="course-stat">
                                        <dt>{{ $label }}</dt>
                                        <dd class="text-break {{ $field === 'discount_price' ? 'text-primary' : '' }}">
                                            {{ is_numeric($course->$field) ? number_format((float) $course->$field, 2) : 'Not set' }}
                                        </dd>
                                    </div>
                                </div>
                            @endforeach
                    ...
                    <div class="card-body">
                        ...
                        @if ($course->instructor)
                            <div class="d-flex align-items-center gap-3 mb-4">
                                <div class="instructor-avatar text-primary" aria-hidden="true">
                                    {{ mb_strtoupper(mb_substr($course->instructor->name, 0, 1)) }}</div>
                                <div class="min-width-zero">
                                    <h6 class="mb-1 text-break">{{ $course->instructor->name }}</h6>
                                    <div class="small text-muted text-break">{{ $course->instructor->email }}</div>
                                </div>
                            </div>
                            <a href="{{ route('admin.instructor.edit', $course->instructor->id) }}"
                                class="btn btn-outline-primary w-100">Manage Instructor <i
                                    class="bx bx-right-arrow-alt ms-2" aria-hidden="true"></i></a>
                        @else
